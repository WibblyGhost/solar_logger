...
import private.influx_codenames
...
def run_int_query(query: QueryBuilder):
    ...
    query_result = run_query(influx_db=INFLUX_DB, query=str(query), query_mode=query_mode)
...
def run_default():
    ...
    q_b = QueryBuilder(bucket=INFLUX_DB.influx_bucket, start_range='-1')
    q_b.append_filter('_measurement', 'fx-1', 'or')
    q_b.append_filter('_measurement', 'mx-1')
...
def main():
    ...
    """
    # print('Build a query using QueryBuilder() run QueryBuilder.help() for info.\n'
    #         '***************\n'
